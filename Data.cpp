...
void Data::erase(Student* s) {
    ...
    for (auto& g : groups)
        for (auto it = g.second.begin(); it != g.second.end(); ++it)
            if (s->id() == (*it)->id()) {
                g.second.erase(it); // changes the underlying vector but never
                                    // continues looking through
                return;
            }
}
...
std::string Data::printStudents(const ull& gid) {
    ...
    if (g == groups.cend())
        return "No such group #" + std::to_string(gid) + " exists\n";
    t << fort::header << "Name"
      << "Events"
      << "Degree"
      << "Birthdate"
    ...
    for (const auto& s : g->second) {
        ...
        for (const auto& l : s->links) {
            i++;
            ev << delim << i << '.'
               << l->getName(); // Add numbers to select something later
            delim = '\n';
        }
        t << s->getName() << ev.str() << s->degree << s->birthdate
          << s->getAge() << s->avgGrade << (s->isTuition ? "Yes" : "No")
          << s->id() << fort::endr;
    }
...
std::string Data::printEvents() {
    ...
    for (const auto& e : events) {
        ...
        t << e->getName() << ss.str() << e->place << e->date << e->id()
          << fort::endr; // Output
    }
...
bool Data::delAccount(const std::string& l, const bool& isadmin) {
    if (isadmin) // For admins and users
    {
        ...
        if (sought == admins.end() || admins.size() < 2)
            return false; // Can't delete the last account
        admins.erase(sought);
    } else {
        auto sought = users.find(l);
        if (sought == users.end())
            return false; // Can delete all
        users.erase(sought);
    }
...
std::vector<Entry*> Data::sieve(ull                gid,
                                const std::string& str) // Search anything
{
    std::vector<Entry*> ret; // Create the result
    ...
    if (gr != groups.end()) {
        for (auto& el : gr->second)
            if (el->checkLinks(str))
                ret.push_back(el.get());
    } else
        plog->put("Couldn't find group #", gid);
    return ret;
}
...
void Data::load() try // Try-catch function block
{
    ...
    auto        fexc = [&name](const std::string& what) {
        throw std::invalid_argument("File: " + name + " couldn't read " + what);
    };
    ...
    {         // USER
        if (eof())
            break; // If the file has ended break out
        if (!readString(f, tempA, 'n'))
            fexc("login");
        if (!readString(f, tempB, 'n'))
            fexc("password");
    ...
    {
        std::cerr << "Warning! We  couldn't find any valid administrator "
                     "accounts. \n" // We MUST have at least one.
                     "Created a new one: admin | admin"
                  << std::endl;
    ...
    while (f) { // id, name, date, place
        if (eof())
            break;
        if (!readString(f, tempA, 'i'))
            fexc("ID");
        if (!readString(f, tempB, 's'))
            fexc("name");
        if (!readString(f, tempC, CHECK::DATE))
            fexc("date");
        if (!readString(f, tempD, 's'))
            fexc("place");
    ...
    std::string                               gid;
    std::pair<decltype(groups.begin()), bool> group =
        std::make_pair(groups.end(), 0);
    while (f) { // id, name, degree, date, tuition, grade
        ...
        while (!eof()) {
            ...
            if (tempA[0] == '#') {
                ...
                tempA.erase(0, 1);
                if (!checkString(tempA, 'i').first)
                    fexc("Group ID");
                gid   = tempA;
                group = addGroup(stoid(gid));
                plog->put((group.second ? "Added group" : "Couldn't add group"),
                          gid);
                break;
            }
            if (!checkString(tempA, 'i').first)
            ...
            if (!readString(f, tempB, 's'))
                fexc("name");
            if (!readString(f, tempC, 's'))
                fexc("events");
            if (!readString(f, tempD, 's'))
                fexc("degree");
            if (!readString(f, tempE, CHECK::DATE))
                fexc("date");
            if (!readString(f, tempF, 'b'))
                fexc("tuition");
            if (!readString(f, tempG, 'f'))
                fexc("grade");
            Student* ps = addStudent(stoid(gid), stoid(tempA), tempB, tempD,
                                     tempE, std::stoi(tempF), std::stof(tempG));
            std::stringstream ss(tempC);
            while (getline(ss, tempC, ',')) { // May change it
                if (!checkString(tempC, 'i').first)
                    fexc("student's event ID");
                ull  sid    = stoid(tempC);
                auto sought = std::find_if(events.begin(), events.end(),
                                           [&sid](std::unique_ptr<Event>& pev) {
                                               return sid == pev->id();
                                           }); // Search for that ID and save
                plog->put(
                    "sought.first is: ",
                    (sought != events.end() ? (*sought)->getName() : "NULL"));
                if (sought == events.end()) // If we didn't find anything
                {
                    plog->put("Creating new unknown event and linking with",
                              ps->getName());
                    addEvent(stoid(tempD), "Unknown event", "0.0.0000",
                             "Unknown")
                        ->link(ps); // Handle the error, create a blank entry
                } else {
                    plog->put("Linking", (*sought)->getName(), "+",
                              ps->getName());
    ...
    std::cerr << "Error while reading files. The program cannot continue."
              << std::endl;
