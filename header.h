...
// Base class
/*
 * Each journal entry represents some kind of table row that contains different
 * info for each entry, but the operations are same. This is some kind of merged
 * interface and usual base class. Will be used with dynamic binding later
 * extensively, so I tried to provide A lot of virtual functions to minimize
 * dynamic casts and RTTI which are slow
 */
class Entry {
    friend class Data;
    friend std::ostream& operator<<(std::ostream& os,
                                    const Entry& e) { // dynamic binding here
        os << e.to_string(); // Each type prints its own info, virtual
        return os;
    }
    friend bool operator==(
        const Entry& lhs, const Entry& rhs) // For some containers and functions
    {
        plog->put("called", lhs.getName(), "==", rhs.getName());
        return lhs.id() == rhs.id();
    } // applicable to any entry because of dynamic binding
public:
    Entry()               = delete; // No blank entries
    Entry(const Entry& e) = delete; // No copies
    //    Copying an entry doesn't make sense. Why would we need several
    //    identical lines in our journal?
    virtual ~Entry();
    [[nodiscard]] const ull& id() const {
        return unique_id;
    } // We shouldn't discard the returned value for the getter. Would make no
      // sense. Just for safety
...
protected: // Constructors are protected to disallow users creating entries in
           // the journal. It's unsafe and makes no sense. Users will use the
    // Appropriate functions from Data
    [[nodiscard]] virtual std::string to_string() const                 = 0;
    [[nodiscard]] virtual bool        check(const std::string& s) const = 0;
    explicit Entry(const ull& id, std::string n) :
        unique_id(id), name(std::move(n)) {
...
public:
    ...
    [[nodiscard]] unsigned short getAge() const {
        auto age = getCurYear() -
                   stoi(birthdate.substr(birthdate.find_last_of('.') + 1, 4));
...
private:
    // CONSTRUCTOR
    explicit Student(const ull& no, const std::string& n, std::string d,
                     std::string b, bool i, float gr) :
        Entry(no, n),
        degree(std::move(d)), birthdate(std::move(b)), isTuition(i),
        avgGrade(gr) {}
...
private:
    // CONSTRUCTOR
    explicit Event(const ull& no, const std::string& n, std::string d,
                   std::string pl) :
        Entry(no, n),
        place(std::move(pl)), date(std::move(d)) {}
    [[nodiscard]] std::string to_string() const override;
    [[nodiscard]] bool        check(const std::string& s) const override;
    std::string               place;
    std::string               date;
...
public:
    ...
    ~Data() {
        events.clear(); // To ensure we don't call a pure virtual, we need a
                        // strict order.
        groups.clear();
    }
    static Data*
    getInstance() // Returns a reference to the single static instance of Data.
    {
        static Data instance; // The instance is always one and lazy-evaluated
                              // on the first use
    ...
    Student* addStudent(const ull& gid, const ull& id, const Args&... args) {
        ...
        return groups[gid]
            .emplace_back(new Student(id, args...))
            .get(); // TODO: Don't return raw pointers
    }
    template <typename... Args>
    ...
    std::vector<Entry*>
         sieve(ull, const std::string& str); // Checks the events in the group
    ...
    bool        passCheck(const std::string& l, const std::string& p,
                          const bool& isadmin) {
        return ((isadmin ? admins.find(l) : users.find(l))->second == hash(p));
    }
    bool loginCheck(const std::string& s,
                    const bool& isadmin) // Ensure the user exists
    {
        return (isadmin ? admins.find(s) != admins.end() :
                          users.find(s) != users.end());
    }
    bool addAccount(const std::string& l, const std::string& p,
                    const bool& isadmin) {
    ...
    bool changePass(const std::string& l, const std::string& p,
                    const bool& isadmin) {
...
private:
    ...
    // Groups are stored in a map where the key is the group #, and each group
    // contains a vector of pointers to Students, allocated on the heap
    std::vector<std::unique_ptr<Event>> events;
    std::map<std::string, std::string>
                                       users; // holds <login, password> (hashed)
...
#define USER_CONSOLE_ENTRIES                                                   \
    ...
    "\nq -> Sign off"
#define ADMIN_MANAGEMENT_ENTRIES                                               \
    ":ADMIN:"                                                                  \
    "\nSelect an option: "                                                     \
    "\n1 -> Search anything "                                                  \
    "\n2 -> Show data "                                                        \
    ...
    "\n5 -> Add a new event"                                                   \
    "\nq -> Go back"
#define USER_MANAGEMENT_ENTRIES                                                \
    ":USER:"                                                                   \
    "\nSelect an option: "                                                     \
    "\n1 -> Search anything"                                                   \
    "\n2 -> Show data "                                                        \
    "\nq -> Go back"
#define SHOW_DATA_MENU_ENTRIES                                                 \
    "Select an option: "                                                       \
...
#define EDIT_STUDENT_OPTIONS                                                   \
    ...
    "\n8 -> Delete"                                                            \
    "\nq -> Nothing"
#define SORT_RESULTS_OPTIONS                                                   \
    "How would you like to sort the result?"                                   \
    "\n1 -> By Name Ascending"                                                 \
