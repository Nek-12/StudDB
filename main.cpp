...
ull select(const ull& limit) { // Select from some kind of range, used in search
                               // primarily
    while (true) {
        std::string s;
...
std::vector<Entry*> search() {
    std::vector<Entry*>
        sought; // There can be several results depending on our query.
    while (true) {
        cls();
        std::string s;
        std::cout << "Enter any property of anything to search (case ignored): "
                  << std::endl;
...
Entry* selectEntry() {
    ...
    if (sought.empty())
        return nullptr;     // If nothing go back
    if (sought.size() == 1) // If one entry select it immediately
        return sought[0];
    for (auto it = sought.begin(); it != sought.end();
         ++it) // If more print them and select just one
...
void addEntries(Entry* pe) {
    while (true) {
        if (!yesNo("Add an entry to " + pe->getName() + " ?"))
            break;
        Entry* sought = selectEntry();
        if (!sought)
            break;            // If we found nothing
        if (pe->link(sought)) // We don't care, there are overridden functions
                              // which are going to take care of that for us.
            std::cout << "Successfully linked " << sought->getName() << " and "
                      << pe->getName() << std::endl;
        else
            std::cout << "Please select a valid entry to link with "
                      << pe->getName() << " !" << std::endl;
    }
}
...
Student* newStudent() {
    ...
    if (!data->findGroup(gid)) {
        if (yesNo("Group not found. Add a new one?"))
            data->addGroup(gid);
        else
            return nullptr;
    }
    ...
    grade = stof(temp);
    Student* added =
        data->addStudent(gid, id, name, degree, date, tuition, grade);
    ...
    addEntries(added);
    std::cout << "Successfully added " << added->getName() << " to group #"
              << gid << std::endl;
    pause();
    return added;
}
...
void manageStudent(Student* ps) {
    ...
    while (true) {
        ...
        case '7':
            std::cout
                << "Select an event's #: "
                << std::endl; // Once printed, there are numbers in our table
        ...
        case '8':
            if (yesNo("Delete this record?")) {
                data->erase(ps);
                std::cout << "Erased this record and removed all references."
                          << std::endl;
                pause();
                return;
            } else
...
void manageEvent(Event* pev) { // Specialized actions for every entry
    ...
    while (true) {
        ...
        case '3':
            ...
            pause();
            break;
        case '4': addEntries(pev); break;
        case '5':
            std::cout
                << "Select a participant's #: "
                << std::endl; // Once printed, there are numbers in our table
        ...
        case '6':
            if (yesNo("Delete this record?")) {
                std::cout << "Erased this event and removed all references."
                          << std::endl;
                data->erase(pev);
                pause();
                return;
...
void manageEntry() {           // Uses RTTI to know which entry we are editing.
    ...
    if (typeid(*pe) == typeid(Student))
        ...
        manageEvent(static_cast<Event*>(
            pe)); // Static cast is SAFE since we got the typeid match
}
...
void management(bool isadmin) { // Differentiates between right levels
    while (true) {
        cls();
        std::cout << (isadmin ? ADMIN_MANAGEMENT_ENTRIES :
                                USER_MANAGEMENT_ENTRIES)
                  << std::endl;
        switch (getch()) {
        case 'q': return;
        case '1':
        ...
        case '3':
            if (isadmin)
                manageEntry(); // For admins: create and manage entries
            break;
        case '4':
            if (isadmin)
                newStudent();
...
void manageUsr() // Admins can delete users, but not admins (except own)
{
    ...
    while (true) {
        cls();
        std::cout << data->printCredentials(
            false); // To see which users to delete
        ...
        if (l == "exit")
            return;
        if (!data->loginCheck(
                l, false)) // Check if the user exists before deleting
...
void createAccPrompt(bool isadmin) {
    ...
    if (data->loginCheck(l, isadmin) ||
        data->loginCheck(l, !isadmin)) // We can't have both admin and user with
                                       // the same username.
    {
    ...
    if (!p.first)
        return;
    data->addAccount(l, p.second, isadmin);
...
bool delDialog(const std::string& l, bool isadmin) {
    ...
    while (true) {
        ...
        std::cout << "THIS WILL DELETE YOUR ACCOUNT AND YOU WILL BE LOGGED OFF."
                  ...
                  << ", TO PROCEED OR \"exit\" TO CANCEL." << std::endl;
    ...
    if (!data->delAccount(l, isadmin)) {
        ...
        // Only valid for admins, you have to remove the generated admin | admin
        // acc. Feel free to remove all users.
        pause();
        return false;
    }
    std::cout << "Deleted account  " << l << std::endl;
...
void console(const std::string& usr, bool isadmin) {
    while (true) {
        cls();
        std::cout << (isadmin ? ADMIN_CONSOLE_ENTRIES : USER_CONSOLE_ENTRIES)
                  << std::endl; // Different options for every1
        switch (getch()) {
        case '1': management(isadmin); break;
        case '2': passChange(usr, isadmin); break;
        case '3':
            if (isadmin)
                createAccPrompt(true); // Conditional statements
            else
                continue;
            break;
        case '4':
        ...
        case '5':
            if (isadmin)
                createAccPrompt(false);
            else
                continue;
            break;
        case '0':
            if (delDialog(usr, isadmin))
                return;
            else
                break;
        case 'q': return;
...
void login(bool isadmin) {
    ...
    while (true) // While the user did not enter his password
    {
        ...
        if (pass == "exit")
            return;
        if (data->passCheck(usr, pass,
                            isadmin)) // If the login and password match
...
auto main(int, char* argv[]) -> int try {
    ...
    // Try function block for convenience. Argc is unused, argv is an array of
    // char arrays, each with an argument, first is path
    plog = Log::init(path + "log.txt");
    data = Data::getInstance(); // Assign to our global pointer. For exception
                                // safety
...
#endif
    bool workin = true,
         first  = true; // The first time we don't clear the screen to show the
                        // user the info from data->load()
    ...
    data->save(); // Saving only before exiting only to avoid corrupting the
                  // database
    plog->put("Reached end of the program");
    return EXIT_SUCCESS;
} catch (std::exception& e) { // If an exception is thrown, the program 100%
                              // can't continue. RIP.
    std::cerr << "Critical Error: " << e.what()
              << "\n The program cannot continue. Press any key to exit..."
              << std::endl;
    plog->put("Exception", e.what());
    getch();
    return (EXIT_FAILURE);
} catch (...) { // Sometimes we can get something completely random. In this
                // case we just exit
    std::cerr << "Undefined Error. \n The program cannot continue. Press any "
                 "key to exit..."
              << std::endl;
